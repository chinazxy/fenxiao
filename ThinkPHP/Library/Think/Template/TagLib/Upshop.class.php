...
class upshop extends TagLib {
	protected $tags = array(
		'adv' => array('attr'=>'limit,order,where,item','close'=>1),
		'article' => array('attr'=>'limit,order,where','close'=>1),
                'upshop' => array('attr'=>'sql,key,item,result_name','close'=>1,'level'=>3), // upshop sql 万能标签
	...
	public function _adv($tag,$content){
        ...
        $str = '<?php ';
        $str .= '$pid ='.$pid.';';
        $str .= '$ad_position = M("ad_position")->cache(true,thishop_CACHE_TIME)->getField("position_id,position_name,ad_width,ad_height");';
        $str .= '$result = D("ad")->where("pid=$pid")->order("orderby desc")->cache(true,thishop_CACHE_TIME)->limit("'.$limit.'")->select();';
        $str .= '
...
foreach($result as $'.$key.'=>$'.$item.'):       
    ...
    public function _upshopassign($tag,$content) {
	...
	*/		
	 ...
	 * @return string
	 */
	public function _upshop($tag,$content){
            $sql = $tag['sql']; // sql 语句     
           ...
           // $sql = str_replace(')', '."', $sql);
            ...
            $parseStr   =   '<?php
                                ...
                                if(empty($'.$name.'))
                                {
                                    ...
                                    S("sql_".$md5_key,$'.$name.',thishop_CACHE_TIME);
                                }    
                             ';  
