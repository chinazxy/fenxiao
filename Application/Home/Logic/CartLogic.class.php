...
class CartLogic extends RelationModel
{
     ...
     */
    function addCart($goods_id,$goods_num,$goods_spec)
    {
        $user = session('user');
        $uid = $user['user_id'];
        $goods = M('Goods')->where("goods_id = $goods_id")->find(); // 找出这个商品  
      
        $specGoodsPriceList = M('SpecGoodsPrice')->where(array('goods_id'=>$goods_id,'key'=>$goods_spec))->find(); // 获取商品对应的规格价钱 库存 条码
        $where = array(
            'user_id'=>$user['user_id'],
        );
        $cat_count = M('Cart')->where($where)->count();
      
    /*     if($car_count>=20){
            return array('status'=>-9,'msg'=>'购物车最多只能放20种商品','result'=>'');
        } */
        if(!empty($specGoodsPriceList) && empty($goods_spec))
            return array('status'=>-1,'msg'=>'商品规格错误','result'=>'');
        if(empty($goods))
      ...
      /*   if(($goods['store_count'] < $goods_num))
        ...
        $catr_goods = M('Cart')->where($map)->find(); // 查找购物车是否已经存在该商品\
        if(!empty($catr_goods)){
            $upMap = array(
               'goods_num'=>$goods_num
            );
             if(I('style')==2){
                $upMap = array(
                        'goods_num'=>$goods_num+$catr_goods['goods_num']
                );
              /*   if(($goods['store_count'] < $upMap['goods_num'])){
                    return array('status'=>-4,'msg'=>'商品库存不足','result'=>'');
                } */
            } 
            $result = M('Cart')->where($map)->save($upMap);
            return array('status'=>1,'msg'=>'修改购物车数量成功','result'=>$result,'post'=>$catr_goods);
        }
        $data = array(
                'user_id'         => $uid,   // 用户id
                ...
                'member_goods_price' => $specGoodsPriceList['price'.$user['level']],  // 会员折扣价 默认为 购买价
                'goods_num'       => $goods_num, // 购买数量
                'spec_key'        => $goods_spec, // 规格key
                'spec_key_name'   => $specGoodsPriceList['key_name'], // 规格 key_name
                'bar_code'        => $specGoodsPriceList['bar_code'], // 商品条形码
                'add_time'        => time(), // 加入购物车时间
        );
        $insert_id = M('Cart')->add($data);
        if($insert_id){
            $cart_count = cart_goods_num($uid); // 查找购物车数量
     ...
     */
    function cartList($user = array() , $selected = 0,$mode =0)
    {                   
        $where = " 1 = 1 ";
        if($selected != NULL)
