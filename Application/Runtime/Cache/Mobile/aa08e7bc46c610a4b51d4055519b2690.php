    ...
    <div class="ccshop ccshop-top">
        <div class="ccshop-selectBox">
            <select class="ccshop-select" name="" id="ccshopSelect1">
                ...
                <option value="0"  <?php if($order==0){echo "selected='selected'";} ?>>默认排序</option>
                <option value="1"  <?php if($order==1){echo "selected='selected'";} ?>>价格由低到高</option>
                <option value="2"  <?php if($order==2){echo "selected='selected'";} ?>>价格由高到低</option>
            </select>
            <select class="ccshop-select" name="" id="ccshopSelect2">
                <option value="0" <?php if($cate_id==0){echo "selected='selected'";} ?>>筛选条件</option>
                 <?php if(is_array($cateList)): foreach($cateList as $key=>$cate): ?><option value="<?php echo ($cate["id"]); ?>"<?php if($cate_id==$cate['id']){echo "selected='selected'";} ?>><?php echo ($cate["name"]); ?></option><?php endforeach; endif; ?>
            </select>
            <div id="ccshopBtn" class="ccshop-icon icons-scale icons icons-list-o" style="display:inline-block;margin-bottom:6px;"></div>
        </div>
       <?php if(count($hot_goods)>0){ ?>
        ...
        <a href="<?php echo U('Mobile/Goods/productDetails');?>?id=<?php echo ($hot["goodsid"]); ?>"><img class="ccshop-img" src="<?php echo ($hot["original_img"]); ?>" alt=""></a>
            <div class="ccshop-content">
                ...
                <div class="ccshop-status">
                    ...
                    <div class="ccshop-car icons icons-car"></div><input type='hidden' value='<?php echo ($hot["goodsid"]); ?>'/>
                </div>
            </div>
        ...
        <div class="ccslider-content">
            <div class="ccslider-shop">
               <div class="ccslider-imgBox">
                   <img class="ccslider-img" src="/Public/jd/img/shop.jpg" alt="">
               </div>
        ...
        $(function(){
           /* 商店 切换形态 */
            ...
            $(".load-download").on("click",function(){
                $.get("<?php echo U('brandGoodsList');?>",{page:page,id:"<?php echo ($id); ?>",order:"<?php echo ($order); ?>",cate_id:"<?php echo ($cate_id); ?>",keyword:"<?php echo ($keyword); ?>"},function(data){
                    if(data.status==1001){
            ...
            function createCcshopItem(arrs){
                arrs.map(function(res,num){
                    ...
                    var $link = $("<a />",{href:"<?php echo U('Mobile/Goods/productDetails');?>?id="+res.goodsid});
            ...
            $("#ccshopSelect1").on("change",function(){
                /* 筛选规则 */
                var order = $(this).val();
                location.href="<?php echo U('brandGoodsList');?>?id=<?php echo ($id); ?>&keyword=<?php echo ($keyword); ?>&cate_id=<?php echo ($cate_id); ?>&order="+order;
                return;
            ...
            $("#ccshopSelect2").on("change",function(){
                /* 筛选规则 */
                var order = $(this).val();
                location.href="<?php echo U('brandGoodsList');?>?id=<?php echo ($id); ?>&keyword=<?php echo ($keyword); ?>&order=<?php echo ($order); ?>&cate_id="+order;
                return;
/*                 var optionParren=$(this).children("option:selected").val();
                if(flag4){
                    flag4=false;
