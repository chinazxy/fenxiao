...
class ReportController extends BaseController{
	...
	public function saleTop(){
		$sql = "select goods_name,goods_sn,sum(goods_num) as sale_num,sum(goods_num*member_goods_price) as sale_amount from __PREFIX__order_goods ";
		$sql .="  where order_id in (select order_id from __PREFIX__order where add_time>$this->begin and add_time<$this->end and pay_status=1  ) group by goods_id order by sale_amount DESC limit 100";
	...
	public function userTop(){
		...
		$sql .= "  on a.user_id = b.user_id where a.add_time>$this->begin and a.add_time<$this->end and a.pay_status=1 group by a.user_id order by amount DESC limit 100";
