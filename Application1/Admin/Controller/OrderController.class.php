...
class OrderController extends BaseController {
    ...
    public function ajaxuser(){
        ...
        $show = $Page->show();
        //获取订单列表
        $orderList = $orderLogic->getOrderList($condition);
        foreach ($orderList as $key => $val) {
            $tmp = M('Users')->field('mobile')->find($val['user_id']);
            $orderList[$key]['user_mobile'] = $tmp['mobile'];
        }
    ...
    public function ajaxindex(){
        ...
        //获取订单列表
            ...
            foreach ($orderList as $key => $val) {
                $tmp = M('Users')->field('mobile')->find($val['user_id']);
                $orderList[$key]['user_mobile'] = $tmp['mobile'];
                $pay_name = M('account_log')->where('order_id ='.$orderList[$key]['order_id'])->find();
                $orderList[$key]['pay_name'] = $pay_name['desc'];
            }
        $this->assign('orderList',$orderList);
        $this->assign('page',$show);// 赋值分页输出
        $this->display();
    ...
    */
    ...
    public function detail($order_id){
        ...
        $dec = M('account_log')->where('order_id = '.$order_id)->find();
        // 获取操作记录
        $action_log = M('order_action')->where(array('order_id'=>$order_id))->order('log_time desc')->select();
		$info = M("delivery_doc")->where('order_id ='.$order_id)->find();
        $this->assign('dec',$dec);
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
			...
			$update['order_amount'] = $update['shipping_price'] + $update['discount']  + $order['total_amount'];
    ...
    public function delivery_info(){
    	...
    	$order = $orderLogic->getOrderInfo($order_id);
    	$orderGoods = $orderLogic->getOrderGoods($order_id);
		$delivery_record = M('delivery_doc')->where('order_id='.$order_id)->find();
		$order['invoice_no'] = $delivery_record['invoice_no'];
		/*print_r($delivery_record);exit;*/
    	$this->assign('order',$order);
    	$this->assign('orderGoods',$orderGoods);
		$this->assign('log',$delivery_record);//发货记录
        //print_r($delivery_record);exit;
    	$this->display();
    }
    ...
    public function ajax_return_list(){
        ...
        $status =  I('status');
        if($order_sn !=''){
            $sql = "select * from up_return_goods where order_sn =".$order_sn;
            $list = M('return_goods')->query($sql);
        }
        elseif($status != 1 ){
            $sql = "select * from up_return_goods where status =".$status;
            $list = M('return_goods')->query($sql);
        }else{
            //$where = " 1 = 1 ";
            ...
            $where1['parent_id'] =1;
            $count = M('return_goods')->where($where1)->count();
            $Page  = new AjaxPage($count,13);
            $show = $Page->show();
            $list = M('return_goods')->where($where1)->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
        }
        foreach($list as $k=>$v){
            $goods_info = json_decode($list[$k]['goods_info'],true);
            foreach($goods_info as $key=>$val){
                $goods_name = M('goods')->field('goods_name')->where(array('goods_id'=>$goods_info[$key]['goods_id']))->find();
                $goods_info[$key]['goodinfom'] = $goods_name ;
            }
    ...
    public function return_del(){
        ...
        $goods = json_decode($return_goods['goods_info'],true);
        foreach($goods as $k=>$v){
            M('user_goods')->where(array('uid' =>$return_goods['user_id'],'goods_id'=>$v['goods_id']))->setInc('stock',$v['goods_num']);
        }
        $sql = "update up_order set order_status = 8 where user_id = $return_goods[user_id] and order_id = $return_goods[order_id]";
        M('order')->execute($sql);
        M('return_goods')->where("id = $id")->delete();
        adminLog('删除某个退换货申请',__ACTION__);
        $this->success('成功拒绝退货申请!');
    }
    /**
     * 退换货操作
     */
    ...
    public function return_info() {
        ...
        $goods = json_decode($return_goods['goods_info'], true);
        $return = json_decode($return_goods['goods_info'], true);
        foreach ($return as $key => $val) {
            $return['return_money'] += $val['goods_num'] * $val['goods_price'];
        }
        if ($return_goods['imgs'])
            $return_goods['imgs'] = explode(',', $return_goods['imgs']);
        $sql = "SELECT og.*,g.original_img FROM up_order_goods og LEFT JOIN up_goods g ON g.goods_id = og.goods_id WHERE order_id = $return_goods[order_id]";
        $array = M('order_goods')->where(array('order_id'=>$return_goods[order_id]))->select();
        foreach ($array as $k => $v) {
            foreach ($return as $kk => $vv) {
                if ($vv['goods_id'] == $v['goods_id']) {
                    $array[$k]['return_num'] = $vv['goods_num'];
                }
            }
        }
        $user = M('users')->where(array('user_id'=>$return_goods[user_id]))->find();
        $type_msg = array('退换', '换货');
        $status_msg = array('未处理', '已完成');
        if (IS_POST) {
            ...
            if ($_POST['status'] == 2) {
                ...
                $result = M('return_goods')->where(array("id"=>$_POST['id']))->save($data);
                foreach ($goods as $val) {
                    //$res1 = M('user_goods')->where("goods_id = $val[goods_id]")->setInc('store_count',$val['goods_num']);//更改库存
                    mysql_query("BEGIN");
                    $res1 = M('user_goods')->where("uid=1 and goods_id = $val[goods_id]")->setInc('stock', $val['goods_num']); //更改用户库存
                    if ($res1) {
                        $res2 = M('goods')->where("goods_id = $val[goods_id]")->setInc('store_count', $val['goods_num']); //更改商品库存
                        $res3 = M('SpecGoodsPrice')->where("goods_id = $val[goods_id]")->setInc('store_count', $val['goods_num']); //更改规格库存
                        mysql_query('COMMIT');
                    } else {
                ...
                //是否线下 
                $payfrom = M('order')->field('payfrom')->where(array('order_sn' => $return_goods['order_sn']))->find();
                if ($payfrom['payfrom'] == 1) {
                    $sql_user_child = "update up_users set user_money = user_money + $return[return_money]  where user_id = $return_goods[user_id]"; //加钱
                    $sql_user_child2 = "update up_users set user_money = user_money - $return[return_money] where user_id = $return_goods[parent_id]";
                    mysql_query("BEGIN");
                    $result1 = M('users')->execute($sql_user_child);
                    if ($result1) {
                        $result2 = M('users')->execute($sql_user_child2);
                        mysql_query('COMMIT');
                    } else {
                        mysql_query('ROLLBACK');
                    }
                ...
                if ($result) {
                    ...
                    $res = M('order')->where(array('order_id'=>$return_goods['order_id']))->save($data_order); //更改订单状态
                    $type = empty($data['type']) ? 2 : 3;
                    //$where = " order_id = " . $return_goods['order_id'] . " and goods_id=" . $return_goods['goods_id'];
                    M('order_goods')->where(array('order_id'=>$return_goods['order_id'],'goods_id'=>$return_goods['goods_id']))->save(array('is_send' => $type)); //更改商品状态
                    $orderLogic = new OrderLogic();
                    $order_id = $return_goods['order_id'];
                    $log = $orderLogic->orderActionLog($order_id, 'refund', $note);
                    adminLog('退换货操作', __ACTION__);
                    $this->success('修改成功!');
                    exit;
                }
            } else {
                $result22 = M('return_goods')->where(array("id"=>$_POST['id']))->save(array('remark' => $data['remark']));
                if ($result22) {
                    $this->success('修改成功!');
                    exit;
                } else {
   ...
   {                
            ...
            adminLog('管理员生成申请退货单',__ACTION__);
    ...
    public function export_order()
    {
    	...
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">发货快递</td>';
    	...
    	foreach($orderList as $k=>$val){
            ...
            if($val['pay_name'] ==''){
                $strTable .= '<td style="text-align:left;font-size:12px;">未支付</td>';
            }else{
