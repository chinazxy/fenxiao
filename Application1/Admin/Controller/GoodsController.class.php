...
class GoodsController extends BaseController {
    ...
    public function ajaxGoodsList(){            
        ...
        */
        ...
        foreach ($goodsList as $key=>$val){
           ...
           if(!empty($val['cat_id'])){
               $a =  D('goods_category')->field("group_concat(name) as name")->where('id in('.$val['cat_id'].')')->select();
               $goodsList[$key]['cate_name'] =$a[0]['name'];
           }
        }
    ...
    public function addEditGoods(){
            ...
            {      
                ...
                if(!$Goods->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                    ...
                    if ($type == 2)
                    {
                        ...
                        $Goods->save(); // 写入数据到数据库
                        $sql ="update up_goods set cat_id='{$_POST['cat_id']}' where goods_id = {$goods_id}";
                        //M('user_goods')->where('uid = 1')->where('goods_id = '.$goods_id)->save();
                        M('Goods')->execute($sql);
                     ...
                     * 需要对item数据进行处理
                     */
                    $userGoodsMod= M('UserGoods');
                    $item = I('item');
                    if(is_array($item)){
                        foreach ($item as $key=>$val){
                            $usegoodsMap = array('uid'=>1,'goods_id'=>$goods_id);
                            $rest =  $userGoodsMod->where($usegoodsMap)->find();
                            $useGoodData = array(
                               'stock'=>$val['store_count'],
                               'up_time'=>time()
                            );
                            if($rest){
                               $returns =  $userGoodsMod->where($usegoodsMap)->save($useGoodData);
                              ...
                              /*   $mapts = array(
                                ...
                                $returns =$userGoodsMod->add($useGoodData);
                            }
                        }
                    }
                    ...
                    adminLog("添加修改商品",__ACTION__);
                    $this->ajaxReturn(json_encode($return_arr));
                }  
            }
            $goodsInfo = D('Goods')->where('goods_id='.I('GET.id',0))->find();
            $goodsInfo['cat_id'] = explode(',', $goodsInfo['cat_id']);
            //$cat_list = $GoodsLogic->goods_cat_list(); // 已经改成联动菜单            
            $level_cat = $GoodsLogic->find_parent_cat($goodsInfo['cat_id']); // 获取分类默认选中的下拉框           
            $cat_list = M('goods_category')->where(array('is_show'=>1))->select(); // 已经改成联动菜单            
            $brandList = $GoodsLogic->getSortBrands();
           ...
           // $this->assign('goodsType',$goodsType);
            $this->assign('goodsType',$cat_list);
 ...
 */
    public function formAccount(){
        //$list = M('UserGoods')->where('uid= 1')->join('up_goods ON up_goods.goods_id = up_user_goods.goods_id')->select();
    ...
    public function change_stock(){
        ...
        mysql_query("BEGIN");
        $t = M('UserGoods')->where($map)->save($data);
        if($t){
			$t1 = M('goods')->where("goods_id = $map[goods_id]")->save($data1);
            $t2 = M('SpecGoodsPrice')->where("goods_id = $map[goods_id]")->save($data1);
            mysql_query('COMMIT');
        }else{
            ...
            {                
                ...
                if(!$model->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                   ...
                   // C('TOKEN_ON',true); //  form表单提交
                    ...
                    adminLog("添加修改编辑商品属性",__ACTION__);
                    $this->ajaxReturn(json_encode($return_arr));
                }  
            }                
           // 点击过来编辑时                 
    ...
    public  function addEditBrand(){        
            ...
            if(IS_POST)
            {
                    ...
                    adminLog("添加修改编辑商品品牌",__ACTION__);
    ...
    {        
        ...
        adminLog("删除品牌",__ACTION__);   //$return_arr = array('status' => -1,'msg' => '删除失败','data'  =>'',);        
        $this->ajaxReturn(json_encode($return_arr));
    }      
    ...
    public function ajaxSpecList(){ 
        ...
        $this->assign('page',$show);// 赋值分页输出
        $goodsTypeList = M("GoodsCategory")->select(); // 规格分类
            ...
            {                
                ...
                if(!$model->create(NULL,$type))// 根据表单提交的POST数据创建数据对象                 
                {
                   ...
                   // C('TOKEN_ON',true); //  form表单提交
                    ...
                    adminLog("添加修改编辑商品规格",__ACTION__);
                    $this->ajaxReturn(json_encode($return_arr));
                }  
            }                
           // 点击过来编辑时                 
