...
class UsersLogic extends RelationModel
{
     ...
     * 登陆
     */
    public function login($username, $password,$only){
        //根据信购唯一标识符进行判断是否存在，存在则直接后台执行登录方法
        $result = array();
        $Only = M('users','uc_')->where('only = '.$only)->select();
        if($Only){
            $result = array('status' => 1, 'msg' => '登录成功', 'result' => $Only);
        }
       elseif(!$username || !$password)
           ...
           if (!$user) {
               ...
               curl_setopt($ch, CURLOPT_URL, 'http://indexgd.wei580.com/PasswordSaltHash.aspx');
               $info = curl_exec($ch);
               curl_close($ch);
           ...
           elseif($user['is_temination'] == 1){
               $result = array('status' => -2, 'msg' => '您已经与平台解约，无法登录！');
           }
           elseif ($user['is_lock'] == 1) {
               $result = array('status' => 1, 'msg' => '未缴纳保证金！！！', 'result' => $user);
           } else {
               $result = array('status' => 1, 'msg' => '登录成功', 'result' => $user);
           }
        return $result;
    ...
    public function get_children($uid){
        $usersMod = M('users');
        return $usersMod->where(array('parent_id'=>$uid))->select();
    }
    /*
     * 第三方登录
     */
    ...
    public function reg($username,$password,$password2){
     ...
     public function get_info($user_id){
        ...
        if(!$user_info)
         ...
         $levelname = M('UserLevel')->where(array('level_id'=>$user_info['level']))->find();
         $user_info['levelname'] = $levelname['level_name'];
...
//        $type = I('get.type',0);
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        if(!$post['province'])
            return array('status'=>-1,'msg'=>'所在省份不能为空','result'=>'');
        if(!$post['address'])
    ...
    public function confirm_order($user_id,$id){        
        ...
        if($order['order_status'] ==2){
            ...
            foreach ($order_goods as $key => $val) {
                ...
                $t = M('user_goods')->where($maps)->find();
                if (empty($t)) {
                    $maps['up_time'] = time();
                    $maps['stock'] = $val['goods_num'];
                    $rest = M('UserGoods')->add($maps);
                } else {
                    $maps['up_time'] = time();
                    $maps['stock'] = $t['stock'] + $val['goods_num'];
                    $rest = M('UserGoods')->where(array('uid' => $user_id, 'goods_id' => $val['goods_id'], 'spec_id' => $val['spec_key']))->save($maps);
                }
            }
            $row = M('order')->where(array('order_id' => $id))->save($data);
        }
        //付款sql
        $up = M('users')->where($order[parent_id])->find();
        if($up['u_type'] == 1) {
            $sql_user = "update up_users set user_money = user_money + $order[order_amount] ,frozen_money = frozen_money - $order[order_amount] where user_id = $order[parent_id]";
            $result = M('users')->execute($sql_user);
            $userData = array(
                'user_id' => $user_id,
        ...
        elseif($up['u_type'] == 2){
            ...
            $sql_user = "update up_users set user_money = user_money + $order[order_amount] ,frozen_money = frozen_money - $order[order_amount] where user_id = $order[parent_id]";
            $result = M('users')->execute($sql_user);
            $ucuser = M('users','uc_')->where($order[parent_id])->find();
            $userData = array(
                'u_id' => $ucuser['u_id'],
                'user_id'=>$order[parent_id],
                'f_log_num' => $order[order_amount],
                'f_logs_type'=>'1',
                'f_logs_datetime' => time(),
                'f_log_info' => '确认收货金额'
            );
            $userAccouontLog = M('finance_logs','uc_')->add($userData);
            if($result && $userAccouontLog){
                mysql_query('COMMIT');
            }else{
