...
<div id="tbh5v0">
    ...
    <form action="<?php echo U('Mobile/order_list/ajax_order_list');?>" name="filter_form" id="filter_form">
        <div class="z-order-list-head clearfix"><a class="col-xs-6" href="<?php echo U('/Mobile/User/order_list');?>">我的订单</a><a class="col-xs-6" style="color:#666;" href="<?php echo U('/Mobile/User/order_list_down');?>">下级订单</a></div>
        <div class="z-order-head clearfix">
            <ul>
                <li><a href="<?php echo U('/Mobile/User/order_list_down');?>" class="tab_head <?php if($_GET[type] == ''): ?>on<?php endif; ?>" >全部</a></li>
                <li><a href="<?php echo U('/Mobile/User/order_list_down',array('type'=>'WAITSEND'));?>"     class="tab_head <?php if($_GET[type] == 'WAITSEND'): ?>on<?php endif; ?>">待发货</a></li>
                <li><a href="<?php echo U('/Mobile/User/order_list_down',array('type'=>'WAITRECEIVE'));?>"  class="tab_head <?php if($_GET[type] == 'WAITRECEIVE'): ?>on<?php endif; ?>">待收货</a></li>
                <li><a href="<?php echo U('/Mobile/User/order_list_down',array('type'=>'RETURNING'));?>"  class="tab_head <?php if($_GET[type] == 'RETURNING'): ?>on<?php endif; ?>">待退货</a></li>
                <!--<li><a href="<?php echo U('/Mobile/User/order_list_down',array('type'=>'WAITPAY'));?>"      class="tab_head <?php if($_GET[type] == 'WAITPAY'): ?>on<?php endif; ?>">待付款</a></li>-->
            </ul>
        </div>
      <style>.z-order-head li{width:25%;}</style>
        <div class="order ajax_return">
            <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><a href="<?php echo U('/Mobile/User/order_detail',array('id'=>$list['order_id'],'type'=>2));?>">
                    ...
                    <?php if(($list["order_status"] > 5)): ?><div class="alert alert-info z-order-list"><?php endif; ?>
                            ...
                            <div class="clearfix">
                                <div class="col-xs-3"><img src="/Public/upload/goods/2016/08-03/57a196524f6c4.jpg"/></div>
                                <div class="col-xs-7 z_kpo">
                                    <div class="text-ellipsis  margin_bottom3"><?php echo ($good["goods_name"]); ?></div>
                                    <div class="font_size13 margin_bottom3">订单金额：￥<?php echo ($list[order_amount]); ?></div>
                                    <div class="font_size13 margin_bottom3">下单时间 &nbsp;&nbsp;<?php echo date("Y-m-d H:i:s",$list[add_time]);?></div>
                                </div>
                                <div class="col-xs-2 z-order-right marginTop30">
                                    <?php if($list[return_money] == '' ): ?><div>￥&nbsp;<?php echo ($list[order_amount]); ?></div><?php endif; ?>
                                    <?php if($list[return_money] != '' ): ?><div>退款金额：￥<?php echo ($list[return_money]); ?></div><?php endif; ?>
                                    ...
                                    <?php if($list["pay_btn"] == 1): ?><div><a class="btn btn-mini" href="<?php echo U('User/pay_order',array('order_id'=>$list['order_id'],'type'=>2));?>">立即付款</a></div><?php endif; ?>
